...
docker run --rm -v $(pwd)/merged:/merged centos:6 yum --installroot /merged -y --releasever 6 install https://dl.fedoraproject.org/pub/epel/epel-release-latest-6.noarch.rpm
...
docker run --rm -v $(pwd)/merged:/merged centos:6 yum --installroot /merged -y --releasever 6 install nano
